...
case "$APP_NAME" in
	...
	pg.sr.status)
		sending_data=$(psql -A --field-separator=' ' -t -h $PGHOST -p $PGPORT -U $PGROLE $PGDATABASE -c  \
			...
			union all \
select '$HOST_NAME', 'psql.primary_server', $TIMESTAMP_QUERY, (select (NOT(pg_is_in_recovery()))::int) \
			union all \
			select '$HOST_NAME', 'psql.standby_server', $TIMESTAMP_QUERY, (select pg_is_in_recovery()::int) \
                        union all
